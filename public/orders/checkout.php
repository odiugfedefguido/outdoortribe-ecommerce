...
$onlyPid = (int)($_POST['only_product_id'] ?? $_GET['only_product_id'] ?? 0);
...
// Carrello (tutto o solo un prodotto)
...
$sql .= " ORDER BY ci.product_id DESC";
...
<head>
  ...
  <style>
    ...
    .checkout-grid{ display:grid; grid-template-columns:2fr 1fr; gap:20px; align-items:start; }
    ...
    input, select{ width:100%; height:42px; padding:0 10px; border:1px solid #ccc; border-radius:10px; font-size:16px; outline:none; }
    ...
    button[type=submit]{ margin-top:.75rem; padding:10px 16px; border-radius:10px; border:0; background:#029664; color:#fff; font-weight:700; cursor:pointer; }
...
<body>
  ...
  <section class="page">
    ...
    <?php if (empty($items)): ?>
      <p>Il carrello selezionato è vuoto o non valido.</p>
      <p><a href="<?= $BASE ?>/public/cart/view.php">Torna al carrello</a></p>
    <?php else: ?>
      <div class="checkout-grid">
        <div>
          <form id="checkoutForm" method="post" action="<?= $BASE ?>/public/orders/place_order.php" novalidate>
            ...
            <?php if ($onlyPid > 0): ?>
              <input type="hidden" name="only_product_id" value="<?= (int)$onlyPid ?>">
            <?php endif; ?>

            <fieldset style="margin-bottom:1rem;">
              <legend>Indirizzo spedizione</legend>
              <label>Nome completo</label>
              <input id="ship_name" name="ship_name" required placeholder="Es. Mario Rossi" pattern="[A-Za-zÀ-ÖØ-öø-ÿ' ]{2,60}">
              ...
              <label>Città</label>
              <input id="ship_city" name="ship_city" required pattern="[A-Za-zÀ-ÖØ-öø-ÿ' -]{2,60}">
  ...
  <script>
    ...
    function setErr(el, id){ el.classList.add('invalid'); const e=document.getElementById(id); if(e) e.style.display='block'; }
    function clrErr(el, id){ el.classList.remove('invalid'); const e=document.getElementById(id); if(e) e.style.display='none'; }
    function luhn(num){ let s=0,a=false; for(let i=num.length-1;i>=0;i--){let n=parseInt(num[i],10); if(a){n*=2;if(n>9)n-=9;} s+=n;a=!a;} return s%10===0; }

    document.getElementById('checkoutForm')?.addEventListener('submit', function(ev){
      let ok=true;
      const name=ship_name, addr=ship_addr, city=ship_city, zip=ship_zip, phone=ship_phone;
      if(!/^[A-Za-zÀ-ÖØ-öø-ÿ' ]{2,60}$/.test(name.value.trim())){ setErr(name,'err_ship_name'); ok=false; } else { clrErr(name,'err_ship_name'); }
      if(addr.value.trim().length<6){ setErr(addr,'err_ship_addr'); ok=false; } else { clrErr(addr,'err_ship_addr'); }
      if(!/^[A-Za-zÀ-ÖØ-öø-ÿ' -]{2,60}$/.test(city.value.trim())){ setErr(city,'err_ship_city'); ok=false; } else { clrErr(city,'err_ship_city'); }
      if(!/^\d{5}$/.test(zip.value.trim())){ setErr(zip,'err_ship_zip'); ok=false; } else { clrErr(zip,'err_ship_zip'); }
      const digits=(phone.value.match(/\d/g)||[]).length, hasLetters=/[A-Za-z]/.test(phone.value);
      if(phone.value && (hasLetters||digits<7||digits>15)){ setErr(phone,'err_ship_phone'); ok=false; } else { clrErr(phone,'err_ship_phone'); }
      if(pm.value==='card'){
        const cn=card_number, exp=card_exp, cvc=card_cvc, raw=cn.value.replace(/\s+/g,'');
        ...
        if(!m){ setErr(exp,'err_card_exp'); ok=false; } else {
          const mm=+m[1], yy=+m[2]; if(mm<1||mm>12){ setErr(exp,'err_card_exp'); ok=false; }
          else { const now=new Date(), year=2000+yy, end=new Date(year, mm, 0, 23,59,59,999); if(end<now){ setErr(exp,'err_card_exp'); ok=false; } else clrErr(exp,'err_card_exp'); }
        }
